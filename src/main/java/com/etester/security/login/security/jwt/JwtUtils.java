...
public class JwtUtils {
	...
	@Value("${etester.app.jwtSecret}")
	private String jwtSecret;

	@Value("${etester.app.jwtExpiration}")
	private int jwtExpiration;

	@Value("${etester.app.jwtCookieName}")
...
//		String jwt = generateJwtTokenFromUsername(userPrincipal.getUsername());
		ResponseCookie cookie = ResponseCookie.from(jwtCookie, jwtToken).path("/api").maxAge(jwtExpiration).httpOnly(true)
				.build();
		return cookie;
	}
...
//        Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(authToken);
			...
			Jws<Claims> var = Jwts.parser().setSigningKey(jwtSecret).parseClaimsJws(authToken);
			System.out.println("Lord Shiva: " + var);
			return true;
